...
from backend.application.use_cases.get.timetable import GetDailyTimetableUseCase
...
async def get_weekly_timetable(
    request: BackendRequest, user_id: str, timetable_dto: TimetableDTO
):
    user = await GetUserByIDUseCase(request.app.ctx.user_repository).execute(
        int(user_id)
    )

    school_info_id = await GetSchoolInfoUseCase(
        request.app.ctx.school_info_repository
    ).execute(
    ...
    return json(
        {
            "results": [[item.to_dict() for item in daily] for daily in weekly],
        }
    )
