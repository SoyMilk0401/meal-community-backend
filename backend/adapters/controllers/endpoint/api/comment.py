...
from backend.application.dtos.comment import CreateCommentDTO, CreateReplyCommentDTO
...
@comment.post("/reply")
@validate(json=CreateReplyCommentDTO, body_argument="create_reply_comment_dto")
@require_auth
async def write_reply(
    request: BackendRequest,
    create_reply_comment_dto: CreateReplyCommentDTO,
    user_id: int,
):
    ...
    comment_entity = Comment(
        content=create_reply_comment_dto.content,
        author=user,
        parent_id=create_reply_comment_dto.parent_id)
    
    await CreateReplyCommentUseCase(request.app.ctx.comment_repository).execute(
            user_id=user_id,
            meal_id=create_reply_comment_dto.meal_id, 
            comment=comment_entity,
